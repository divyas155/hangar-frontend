...
export default function ViewPaymentViewer() {
  ...
  const fetchPayments = useCallback(async () => {
    setLoading(true);
    try {
      const params = { status: 'approved' }; // ✅ only approved payments
      if (startDate instanceof Date && !isNaN(startDate)) params.startDate = startDate.toISOString();
      if (endDate instanceof Date && !isNaN(endDate)) params.endDate = endDate.toISOString();

      const res = await api.get('/payments', { params });
      const data = res.data.map((p, idx) => ({
        ...p,
        id: p._id,
        uploader: p.createdBy?.username || 'Unknown',
        serial: idx + 1
      }));
      setPayments(data);
    } catch (err) {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      <Typography variant="h5" gutterBottom>View Approved Payments</Typography>
        ...
        <LocalizationProvider dateAdapter={AdapterDateFns}>
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={12} sm={2}>
              <Button fullWidth variant="contained" onClick={fetchPayments}>Apply</Button>
            </Grid>
          </Grid>
            ...
            <Paper key={entry.id} variant="outlined" sx={{ p: 2 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <Typography><strong>Paying Authority:</strong> {entry.uploader}</Typography>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Typography><strong>Date:</strong> {format(new Date(entry.date), 'dd/MM/yyyy')}</Typography>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Typography><strong>Payment ID:</strong> {entry.paymentID}</Typography>
                </Grid>
                <Grid item xs={12} sm={6}>
                  <Typography><strong>Amount:</strong> ₹{entry.amount?.toLocaleString()}</Typography>
                </Grid>
                <Grid item xs={12}>
                  <Typography><strong>Description:</strong> {entry.description}</Typography>
                </Grid>
                <Grid item xs={12}>
                  <CommentSection itemId={entry.id} type="payment" />
                </Grid>
              </Grid>
            </Paper>
          ))}
      ...
      <Typography variant="h6" sx={{ mt: 3 }}>
        💰 Total Approved Payment: ₹{totalAmount.toLocaleString()}
      </Typography>
